		...
		<div class="content" :class="isShow?'s--signup':''">
			<div class="form">
				...
				<el-select v-model="schoolId" placeholder="请选择" class="school">
					<el-option v-for="item in school" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
							...
							this.$get("/user/login", {
								...
								if (res.data.status) {
									if (res.data.user.state == 0) {
										let accountOrName = res.data.user.username ? res.data.user.username : res
											.data.user.studentId;
...
<style scoped>
	...
	*:after {
	...
	.img:before {
		...
		width: 900px;
		height: 100%;
		background-image: url('https://www.eurasia.edu/images/22/12/06/1e31agc6yt/20210107165211_86894.jpg');
